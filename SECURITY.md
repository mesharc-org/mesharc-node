...
This policy covers `npm install mesharc` (this repository). A problem in the MeshArc service itself — the app at mesharc.dev or the API at api.mesharc.dev — goes to the same address; the service's own policy is at [mesharc.dev/legal/security](https://mesharc.dev/legal/security).
